...
这是一个初学者非常容易踩的“坑”，但理解它之后，你对C++的I/O会有更深刻的认识。

#### 什么是输入缓冲区？

**输入缓冲区**（Input Buffer）是位于键盘和`cin`之间的一块临时内存区域。
*   想象它是一条传送带。当你通过键盘输入内容并按下回车键时，你输入的所有内容（包括换行符`\n`，也就是用户最后按下的回车键）都会被先放到这条传送带上。
*   `cin >>`就像一个只取特定货物的机器人。它从传送带上拿走它需要的东西（比如一个数字），但会把剩下的东西（比如空格和回车符）留在传送带上。

#### 问题显现

看下面的代码，我们想先读取一个数字，再读取一行包含空格的姓名。
```cpp
// 错误示范代码
#include <iostream>
#include <string> // 需要引入string头文件来使用string类型

using namespace std;

int main() {
    int id;
    string name; // string是一个特殊的数据类型，用于存储一串文本

    cout << "请输入您的ID号: ";
    cin >> id; // cin读取了ID，但把换行符留在了缓冲区

    cout << "请输入您的全名: ";
    // getline会读取缓冲区中直到回车符之前的所有内容
    getline(cin, name); // getline看到缓冲区里最前面有个现成的回车符，以为输入结束

    cout << "ID: " << id << endl;
    cout << "姓名: '" << name << "'" << endl; // 你会发现姓名是空的
...
**运行交互示例：**
```
请输入您的ID号: 101  <-- 用户输入101，按回车
请输入您的全名: ID: 101
姓名: ''
```
程序没有等待用户输入姓名就直接结束了！这是因为`cin >> id;`执行后，数字`101`被取走，但回车符`\n`还留在“传送带”上。紧接着的`getline`一检查传送带，发现上面有`\n`，它就认为用户输入了一个空行，于是立即结束读取。

#### `getline`函数和解决方案

`getline(cin, 变量)`函数专门用于读取一整行文本，直到遇到回车符为止（它会读取并丢弃回车符）。

**解决方案**：在使用`getline`读取之前，我们需要手动清空缓冲区中遗留的回车符。

*   **`cin.ignore()`**：这个函数可以忽略（读取并丢弃）缓冲区中的字符。我们可以用它来清除掉那个多余的回车符。

```cpp
// 正确的代码
#include <iostream>
#include <string>
#include <limits> // 需要这个头文件来使用numeric_limits
...
```
**`cin.ignore()`的解释**：`cin.ignore(numeric_limits<streamsize>::max(), '\n')`是一个固定用法，它的意思是“请忽略输入缓冲区中的字符，最多忽略一个超大的数量，直到遇到换行符`\n`为止”。简单记：**它就是用来清理`cin >>`后续`getline`问题的“清道夫”**。
...
*   **优先级**：决定了不同运算符之间的计算顺序。优先级高的运算符会先于优先级低的运算符进行计算。这和我们小学数学里“先乘除，后加减”的规则是一样的。
*   **结合性**：当一个表达式中有多个相同优先级的运算符时，结合性决定了它们的计算方向（是从左到右，还是从右到左）。

#### 常见运算符优先级

下面是一张简化的C++运算符优先级表（由高到低）。你**不需要**死记硬背它，但了解基本顺序对于避免错误非常有帮助。

| 优先级 | 类别/运算符 | 结合性 | 示例 |
|:---:|:---|:---:|:---|
| 1 (最高) | `()` (函数调用/分组) `++` `--` (后置) | 从左到右 | `func()` `x++` |
| 2 | `!` (逻辑非) `++` `--` (前置) | 从右到左 | `!flag` `++x` |
| 3 | `*` `/` `%` (乘、除、取模) | 从左到右 | `a * b / c` |
| 4 | `+` `-` (加、减) | 从左到右 | `a + b - c` |
| 5 | `<` `<=` `>` `>=` (关系) | 从左到右 | `a > b` |
| 6 | `==` `!=` (相等/不等) | 从左到右 | `a == b` |
| 7 | `&&` (逻辑与) | 从左到右 | `cond1 && cond2` |
| 8 | `\|\|` (逻辑或) | 从左到右 | `cond1 \|\| cond2` |
| 9 | `?:` (条件运算符) | 从右到左 | `c ? x : y` |
| 10 (最低) | `=` `+=` `-=` 等赋值运算符 | 从右到左 | `a = b = 10` |
...
你不一定会记住所有运算符的优先级。即使记住了，别人在阅读你的代码时也可能感到困惑。因此，你一定要**使用括号 `()`！**

括号的优先级是最高的，它可以改变运算顺序，并且能让你的代码可读性大大提高。
...
计算结果: 100 + 200 = 300
```
注意这个例子中巧妙利用了`switch`的穿透特性来同时处理大小写输入。
...
*   对于**有符号数**（`signed`），左边空出的位填充方式取决于编译器（可能是`0`，也可能是原符号位），这称为**算术右移**。**为避免不确定性，进行位运算时，强烈建议使用`unsigned`类型。**
...
* 递归版汉诺塔是**分治思想**的教科书级示例：  
  “先搞定一个较小子问题 → 处理当前层最难的动作 → 再复用子问题的解”。  
* 算法正确且步数最优，但时间复杂度不可避免地为指数级。  
